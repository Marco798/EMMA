...
namespace EMMA_BE.Generated.Controller {
	...
	public class SYST_COMBO_VALUEController : ControllerBase {
		...
		[HttpGet("SelectAll")]
		public IActionResult SelectAll(List<SYST_COMBO_VALUE_Field>? fields = null) {
			try {
				List<SYST_COMBO_VALUE_Record> _Record_List = _query.SelectAll(fields);
		...
		public IActionResult SelectWithSimpleCriteria(SYST_COMBO_VALUE_NullRecord nullRecord, List<SYST_COMBO_VALUE_Field>? fields = null) {
			try {
				List<SYST_COMBO_VALUE_Record> _Record_List = _query.SelectWithSimpleCriteria(nullRecord, fields);
				return Ok(_Record_List);
			}
			catch (Exception ex) {
				return BadRequest(ex.Message);
