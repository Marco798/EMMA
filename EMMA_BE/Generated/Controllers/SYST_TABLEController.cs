...
namespace EMMA_BE.Generated.Controller {
	...
	public class SYST_TABLEController : ControllerBase {
		...
		[HttpGet("SelectAll")]
		public IActionResult SelectAll(List<SYST_TABLE_Field>? fields = null) {
			try {
				List<SYST_TABLE_Record> _Record_List = _query.SelectAll(fields);
		...
		public IActionResult SelectWithSimpleCriteria(SYST_TABLE_NullRecord nullRecord, List<SYST_TABLE_Field>? fields = null) {
			try {
				List<SYST_TABLE_Record> _Record_List = _query.SelectWithSimpleCriteria(nullRecord, fields);
				return Ok(_Record_List);
			}
			catch (Exception ex) {
				return BadRequest(ex.Message);
