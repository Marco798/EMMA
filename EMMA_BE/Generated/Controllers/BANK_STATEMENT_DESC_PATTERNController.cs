...
namespace EMMA_BE.Generated.Controller {
	...
	public class BANK_STATEMENT_DESC_PATTERNController : ControllerBase {
		...
		public IActionResult SelectAll_CustomRecord(List<BANK_STATEMENT_DESC_PATTERN_Field>? fields = null) {
			try {
				List<BANK_STATEMENT_DESC_PATTERN_NullRecord> _Record_List = _query.SelectAll(fields);
				return Ok(_Record_List);
			}
			catch (Exception ex) {
				return BadRequest(ex.Message);
		...
		public IActionResult SelectWithSimpleCriteria_CustomRecord([FromQuery] BANK_STATEMENT_DESC_PATTERN_NullRecord nullRecord, [FromQuery] List<BANK_STATEMENT_DESC_PATTERN_Field>? fields = null) {
			try {
				List<BANK_STATEMENT_DESC_PATTERN_NullRecord> _Record_List = _query.SelectWithSimpleCriteria(nullRecord, fields);
				return Ok(_Record_List);
			}
			catch (Exception ex) {
				return BadRequest(ex.Message);
