...
namespace EMMA_BE.Generated.Controller {
	...
	public class BANK_STATEMENTController : ControllerBase {
		...
		[HttpGet("SelectAll")]
		public IActionResult SelectAll(List<BANK_STATEMENT_Field>? fields = null) {
			try {
				List<BANK_STATEMENT_Record> _Record_List = _query.SelectAll(fields);
		...
		public IActionResult SelectWithSimpleCriteria(BANK_STATEMENT_NullRecord nullRecord, List<BANK_STATEMENT_Field>? fields = null) {
			try {
				List<BANK_STATEMENT_Record> _Record_List = _query.SelectWithSimpleCriteria(nullRecord, fields);
				return Ok(_Record_List);
			}
			catch (Exception ex) {
				return BadRequest(ex.Message);
