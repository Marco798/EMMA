...
namespace EMMA_BE.Generated {
	public class SYST_COMBO_Query(IConfiguration configuration) : QueryBase(configuration) {
		...
		public List<SYST_COMBO_NullRecord> SelectAll(List<SYST_COMBO_Field>? fields = null) {
			using SqlConnection connection = new(connectionString);

			List<SYST_COMBO_NullRecord> output = [];
			try {
				connection.Open();
				...
				string query = $"SELECT {selectFields.ToString()[..^2]} FROM SYST_COMBO";
				using (SqlCommand command = new(query, connection)) {
					SqlDataReader reader = command.ExecuteReader();
					while (reader.Read()) {
        ...
        public List<SYST_COMBO_NullRecord> SelectWithSimpleCriteria(SYST_COMBO_NullRecord nullRecord, List<SYST_COMBO_Field>? fields = null) {
            using SqlConnection connection = new(connectionString);
            ...
            try {
                ...
                StringBuilder query = new($"SELECT {selectFields.ToString()[..^2]} FROM SYST_COMBO WHERE ");
                List<SqlParameter> parameters = [];
        ...
        private static SYST_COMBO_NullRecord ReadNullRecord(SqlDataReader reader, List<SYST_COMBO_Field> fields) {
